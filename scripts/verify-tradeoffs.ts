...
const universe: Asset[] = [
  ...
  mkAsset({ id: "a4", ticker: "DEV-D", class: "equity_dev", esg: 88, er: 0.07, vol: 0.15 }),
  // equity_em — 4 assets
  mkAsset({
    id: "b1",
    ticker: "EM-A",
  ...
  mkAsset({ id: "b2", ticker: "EM-B", class: "equity_em", esg: 70, er: 0.085, vol: 0.2 }),
  mkAsset({ id: "b3", ticker: "EM-C", class: "equity_em", esg: 78, er: 0.082, vol: 0.195 }),
  mkAsset({ id: "b4", ticker: "EM-D", class: "equity_em", esg: 85, er: 0.08, vol: 0.2 }),
  // thematic — 4 assets
  mkAsset({ id: "t1", ticker: "TH-A", class: "thematic", esg: 70, er: 0.095, vol: 0.27 }),
  mkAsset({ id: "t2", ticker: "TH-B", class: "thematic", esg: 78, er: 0.085, vol: 0.2 }),
  ...
  // green_bond — 4 assets
  mkAsset({ id: "g1", ticker: "GB-A", class: "green_bond", esg: 80, er: 0.033, vol: 0.06 }),
  mkAsset({ id: "g2", ticker: "GB-B", class: "green_bond", esg: 83, er: 0.032, vol: 0.058 }),
  mkAsset({ id: "g3", ticker: "GB-C", class: "green_bond", esg: 86, er: 0.03, vol: 0.055 }),
  mkAsset({ id: "g4", ticker: "GB-D", class: "green_bond", esg: 88, er: 0.029, vol: 0.052 }),
  // social_bond — 4 assets
  mkAsset({ id: "s1", ticker: "SB-A", class: "social_bond", esg: 76, er: 0.032, vol: 0.06 }),
  mkAsset({ id: "s2", ticker: "SB-B", class: "social_bond", esg: 80, er: 0.031, vol: 0.057 }),
  mkAsset({ id: "s3", ticker: "SB-C", class: "social_bond", esg: 82, er: 0.03, vol: 0.056 }),
  mkAsset({ id: "s4", ticker: "SB-D", class: "social_bond", esg: 84, er: 0.029, vol: 0.054 }),
  // sov_bond — 4 assets
  mkAsset({ id: "v1", ticker: "SV-A", class: "sov_bond", esg: 70, er: 0.029, vol: 0.05 }),
  ...
  mkAsset({ id: "c1", ticker: "CASH", class: "cash", esg: 50, er: 0.02, vol: 0.005 }),
  mkAsset({ id: "r1", ticker: "REIT", class: "reit", esg: 65, er: 0.06, vol: 0.13 }),
...
console.log(
  "  ER  =",
  fmt(baseline.metrics.expected_return),
  "  Vol =",
  fmt(baseline.metrics.volatility),
  "  ESG =",
  baseline.metrics.esg_score.toFixed(2),
);
console.log(
  "  selected =",
  baseline.selected_assets.length,
  "  excluded_count =",
...
function variant(label: string, p: PortfolioParams) {
  ...
  const volDelta = Number(
    ((baseline.metrics.volatility - alt.metrics.volatility) * 100).toFixed(2),
  );
  console.log(`\n--- ${label} ---`);
  console.log(
    "  alt: ER =",
    fmt(alt.metrics.expected_return),
    "  Vol =",
    fmt(alt.metrics.volatility),
    "  ESG =",
    alt.metrics.esg_score.toFixed(2),
  );
  console.log(
    "  selected =",
    alt.selected_assets.length,
    "  poolDeltaFromUniverse =",
    universe.length - alt.excluded_count - (universe.length - baseline.excluded_count),
  );
...
const remFossiles = variant("Sans exclusion fossiles", {
  ...params,
  exclusions: params.exclusions.filter((e) => e !== "fossiles"),
});
console.assert(
  remFossiles.costBps >= 0,
  "I2 violée: costBps doit être >= 0 quand on relâche une exclusion",
);
console.assert(
  remFossiles.alt.excluded_count <= baseline.excluded_count,
  "I1 violée: relâcher une exclusion devrait élargir le pool",
);

// I1+I2 — exclusion armes
const remArmes = variant("Sans exclusion armes", {
  ...params,
  exclusions: params.exclusions.filter((e) => e !== "armes"),
});
...
console.assert(
  riskUp.volDelta <= 0,
  "I4 violée: volDelta = base.vol - alt.vol doit être <= 0 quand on monte la vol cible",
);

// I5 — risk -2pts
const riskDown = variant("Risk -2pts", {
  ...params,
  risk_target: Math.max(0.02, params.risk_target - 0.02),
});
console.assert(riskDown.costBps <= 0, "I5 violée: moins de risque devrait coûter du rendement");
console.assert(
  riskDown.volDelta >= 0,
  "I5 violée: volDelta doit être >= 0 quand on baisse la vol cible",
);
