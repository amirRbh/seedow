/**
 * Chemin OPTIMISEUR — hors du parcours utilisateur.
 *
 * Ces fonctions produisent ou rejouent une allocation calculée : Markowitz
 * contraint, best-in-class, tilts par conviction, backtest walk-forward,
 * rééquilibrage sur une allocation cible. Elles sont conservées pour leur valeur
 * d'ÉVALUATION (valider une méthode, comparer au 1/N) — pas pour composer le
 * portefeuille de quelqu'un.
 *
 * Aucune ne doit être appelée depuis le produit : c'est l'utilisateur qui pose
 * ses poids (`lib/portfolio/weights`, `customize.functions`). Un test de garde
 * (`legacy/__tests__/isolation.test.ts`) échoue si un import remonte vers le
 * chemin produit.
 *
 * ⚠️ `generatePortfolio` et `rebalancePortfolio` ÉCRIVENT en base : les appeler
 * écraserait une composition faite à la main.
 */
...
import type { PortfolioParams, PortfolioResult } from "../types";
...
const ParamsSchema = z.object({
  ...
  /** "replace" (défaut) : désactive les portefeuilles actifs existants. */
