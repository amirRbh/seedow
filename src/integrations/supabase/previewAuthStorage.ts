...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      let done = false;
      // `let` and not `const`: `finish` reads `timer` to clear it, and is
      // defined before the timer exists. A `const` declared after `finish`
      // would put that read in the temporal dead zone. Assigned exactly once.
