    ...
    >
      ...
      <div className="flex gap-2 justify-end">
        <button
          type="button"
          onClick={onCancel}
          className="px-3 py-1.5 rounded-full text-caption text-ink-2 hover:text-ink transition-colors"
        >
          {t("common.cancel")}
        </button>
        <button
...
function Field({
  ...
  return (
    <label className="flex flex-col gap-1">
      <span className="text-tag uppercase tracking-wider text-ink-3">{label}</span>
      <div className="flex items-center gap-1 bg-paper-2 border border-paper-3 rounded-md px-2 py-1.5">
        <input
          type="number"
          value={value}
          step={step}
          ...
          className="w-full bg-transparent outline-none text-sm text-ink font-value"
        />
        <span className="text-tag text-ink-3">{suffix}</span>
      </div>
    </label>
  );
}
