...
export function TimelineEvent({ type, title, subtitle, badge, badgeVariant = "gain", impactChips }: TimelineEventProps) {
  const dotColor = { gain: "bg-moss-1", plant: "bg-moss-2", harvest: "bg-gold", soil: "bg-ink-3" }[type];
  const badgeStyles = {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3.5 pb-3">
