    ...
    <div className="w-full">
      <div className="flex items-baseline justify-between mb-2">
        <p className="text-tag uppercase tracking-[0.12em] text-ink-3 font-medium">
          {t("allocation_viz.allocation_title", { count: holdings.length })}
        </p>
        {rest.length > 0 && (
          <button
          ...
          >
            {expanded
              ? t("allocation_viz.show_less")
              : t("allocation_viz.show_all", { count: holdings.length })}
          </button>
        )}
      </div>
      ...
      <ul className="space-y-1">
        ...
        {!expanded && rest.length > 0 && (
          <li className="flex items-center gap-2.5 py-1.5 text-caption text-ink-3">
            ...
            <span className="flex-1">
              {t("allocation_viz.other_positions", { count: rest.length })}
            </span>
            <span className="tabular-nums w-12 text-right">
              {formatPercent(rest.reduce((s, h) => s + h.allocationPct, 0) / 100, lang, 1)}
            </span>
          </li>
        )}
        ...
        >
          <span className="text-caption tracking-tight">{t("allocation_viz.add_position")}</span>
          <span className="text-tag uppercase tracking-[0.12em]">{t("allocation_viz.explore")}</span>
        </li>
      </ul>
    </div>
  );
