 ...
 * Sources : moyennes long terme MSCI World (TER médian 0.20 %, score MSCI ESG ~52),
 * intensité carbone benchmark ≈ 165 gCO₂e par € investi.
 * Volontairement conservateur pour ne pas biaiser la comparaison.
 */
const MSCI_WORLD = {
  name: "ETF MSCI World",
  ticker: "IWDA / EUNL",
  expectedReturn: 0.072, // 7.2 % annualisé long terme (net dividendes)
  volatility: 0.155, // 15.5 % vol annualisée
  ter: 0.0020, // 0.20 %
  esgScore: 52,
...
function ComparatifPage() {
  ...
  const seedow = {
    ...
    // CO₂ évité (tonnes / 10k€) -> intensité résiduelle estimée vs benchmark
  ...
  // Écart relatif sur 10 ans à capital initial donné
  ...
  // CO₂ évité cumulé sur 10 ans (€/an versé constant non simulé : approx capital constant)
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-paper">
      <div className="max-w-lg mx-auto pb-28">
        ...
        <section className="px-5 pt-6">
          <div className="border-t border-paper-3 pt-5 grid grid-cols-2 gap-4">
            <KPIFigure
              size="sm"
            ...
            <KPIFigure
              ...
              value={`${delta10y >= 0 ? "+" : ""}${delta10y.toLocaleString("fr-FR", {
                minimumFractionDigits: 0,
                maximumFractionDigits: 0,
              })}`}
              unit="€"
        ...
        <section className="px-5 pt-10">
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 border-t border-paper-3 pt-5">
            ...
            <KPIFigure
              ...
              value={`${Math.max(0, (MSCI_WORLD.ter - seedow.ter) * capital).toLocaleString("fr-FR", {
                minimumFractionDigits: 0,
                maximumFractionDigits: 0,
              })}`}
              unit="€/an"
        ...
        <section className="px-5 pt-10">
          <div className="border-t border-paper-3 pt-5">
            ...
            <p className="text-[12px] text-ink-2 leading-relaxed">
              ...
              165 gCO₂e par euro investi (source : MSCI Climate Reports).
            </p>
            <Link
              to="/methodologie"
  ...
  return (
    <div className="py-4">
      ...
      <div className="mt-2 grid grid-cols-2 gap-4">
        <div>
          <p className="text-[10px] uppercase tracking-[0.18em] text-gold font-semibold mb-1">
            Toi
          </p>
          <p className={cn("kpi-figure text-xl tabular-nums", seedowWins ? "text-ink" : "text-ink-2")}>
            {seedowValue}
            {seedowWins && (
              <span aria-hidden="true" className="ml-1.5 text-gold text-xs">●</span>
            )}
          </p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-[0.18em] text-ink-3 font-semibold mb-1">
            MSCI World
          </p>
