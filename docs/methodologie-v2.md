...
1. **`esg_score` non traçable** (estimation maison présentée comme mesure) — viole §1.2 CLAUDE.md. *Bloquant.*
...
Le **Score Seedow** (versionné, SFDR-indépendant — *à conserver, c'est le moat*) agrège **uniquement 2+3**, avec traçabilité obligatoire. SFDR = signal **corroborant, jamais déterminant**.
