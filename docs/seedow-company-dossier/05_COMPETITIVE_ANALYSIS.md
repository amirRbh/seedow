...
> Positionnements décrits de mémoire du marché (janv. 2026 cutoff) — **à revérifier avant un usage investisseur**, les offres évoluent vite. Traiter comme cartographie, pas comme fiche produit certifiée.
...
**Substituts** : Livret A / fonds euros (statu quo de Karim), conseiller bancaire, ne rien faire.
...
1. **« Je ne peux pas y investir pour de vrai. »** Le tueur n°1 : pas d'exécution. Un utilisateur convaincu doit partir ailleurs pour agir.
2. **« Je ne connais pas cette marque avec mon argent. »** Confiance = actif rare ; les concurrents ont l'agrément et l'antériorité.
3. **« Trade Republic me donne déjà des ETF ESG à 0 €. »** Le substitut est gratuit et exécute.
4. **« L'univers est petit. »** ~58 actifs limitent la personnalisation réelle.
5. **« C'est encore une bêta sur liste d'attente. »** Friction + risque perçu de projet non pérenne.

→ La bataille n'est pas « meilleure UX ESG » (Seedow gagne déjà). C'est **exécution + confiance + distribution**. Voir `06` (moat) et `17` (priorités).
