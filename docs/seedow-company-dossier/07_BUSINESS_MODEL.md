...
1. **Court terme (0-12 mois) — Freemium B2C + passerelle courtier.** Monétiser une petite base convaincue via un abonnement clair (aligné ADN), tout en débloquant l'*acte* d'investir par une passerelle courtier (affiliation) → prouve la willingness-to-pay ET l'activation réelle sans agrément. *(OBJECTIF.)*
2. **Moyen terme (12-24 mois) — Licence data ESG / anti-greenwashing en B2B2C.** Transformer le data-engine et l'heuristique en produit vendable à des distributeurs (le vrai levier de moat + marge). *(OBJECTIF.)*
3. **Long terme — % d'encours** *seulement si* la décision d'exécuter en propre est prise (avec le financement et l'agrément que cela suppose — voir `14`).
