...
> Marché initial : **France, francophone, 25-40 ans, ETF-first, sensibilité ESG.** Aligné personas (CLAUDE.md §2) et données AMF (`04`).
...
1. **Contenu / SEO** : les 12 cours + méthodologie publique = actif SEO durable, faible coût marginal, cohérent avec la pédagogie. Cibler les requêtes « ETF ESG », « greenwashing banque X », « où investir responsable ».
2. **PR / autorité anti-greenwashing** : publier des analyses sourcées (le data-engine sert ici). Positionnement « le Yuka de la finance » → earned media, pas de CAC.
3. **Certificat / impact partageable** : boucle organique déjà construite ; à optimiser (k-factor).
4. **Creators finance (FR)** : TikTok/YouTube/Instagram « finance perso responsable » — le public de Léa/Inès y est. Partenariats > pub.
5. **Reddit / communautés** (r/vosfinances), Discord épargne.
6. **B2B2C** (moyen terme) : CSE, mutuelles, néobanques vertes, universités/assos — canal à CAC quasi nul.

**À éviter** : brûler du budget en acquisition payante avant que l'activation et la conversion soient prouvées (`08`).
