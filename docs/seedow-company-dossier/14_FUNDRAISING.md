...
**Recommandation : un pré-seed / love money / subventions ciblé sur les preuves**, puis un vrai seed une fois les 3 signaux obtenus.
...
Au pré-seed (~300 k€, hypothèse) → **~12 mois de runway** au burn minimal. Usage indicatif :

- **Produit & tech** : 30 % (débouché/passerelle, onboarding, data-engine)
- **Data ESG** : 20 % (industrialiser le moat — le vrai différenciateur)
- **Growth / contenu / PR** : 20 % (acquisition organique, autorité anti-greenwashing)
- **Équipe** : 20 % (1-2 profils clés : data/ESG + growth)
- **Juridique / conformité** : 10 % (statut, RGPD, préparation exécution)

## Ce qui débloque un vrai seed

Le point dur si l'option « exécution » est retenue : **agrément (PSI/CIF) + custody + KYC/AML + MiFID II** — c'est un chantier lourd (temps + capital + juridique) qui justifierait un tour plus important, mais **seulement après** preuve d'activation/rétention. Alternative capital-efficient : **passerelle courtier + licence data B2B** (pas d'agrément, meilleure marge) — probablement la bonne première voie.
