...
1. **Absence d'exécution = fait central** : tout le dossier en dépend. Si une intégration courtier/custody existe hors repo, corriger `01`/`07`/`14`. _(Vérifié : aucune trace dans le repo → traité comme FACT « pas d'exécution ».)_
...
- Univers « ~58 » = comptage approximatif de tickers distincts insérés dans les migrations ; le nombre _actif_ en base peut différer (désactivations). Marqué approximatif partout.
