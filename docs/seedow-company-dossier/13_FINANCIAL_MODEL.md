...
> **Modèle conceptuel, pas prévision.** Point de départ réel : **revenu = 0, traction = UNKNOWN.** Toutes les entrées sont des HYPOTHÈSES explicites, à remplacer par des cohortes réelles. Objet : montrer la *forme* de l'économie et ce qui la fait basculer.
...
*(Conversion payante : BEAR 3 % / BASE ~5-6 % / BULL ~10-12 % ; ARPU 40/50/60 € — cf. `08`.)*
...
- **BEAR = zombie** : produit vivant, jamais rentable → scénario le plus probable *si rien ne change* (pas d'exécution, pas de B2B). C'est le risque réaliste, pas le pire.
